...
public class Cache {
	...
	private static int CACHE_MAX_SIZE = 512 * 1024;
	...
	public Cache(String repoServiceUuid, String repositoryUuid, String branchName) {
		this.repositoryPathStr = String.format("%s/%s/.cache/%s/", "WORKPATH", repoServiceUuid, repositoryUuid);
	...
	public void setHead(String headCommitId) {
		try {
			...
			FileUtils.writeStringToFile(file, headCommitId, StandardCharsets.UTF_8, false);
		} catch (Exception ex) {
	...
	public void writeGitDiffToCache(String leftCommitId, String rightCommitId, JSONArray content) {
		...
		try {
			...
			try {
			    ...
			    FileUtils.writeStringToFile(indexFile, idxContent.toString(), StandardCharsets.UTF_8, true);
	...
	public JSONArray readGitDiffFromCache(String leftCommitId, String rightCommitId) {
		...
		try {
		    ...
		    retList = JSONArray.fromObject(new String(buffer.array(), StandardCharsets.UTF_8));
		} catch (Exception ex) {
			logger.error("Read cache from file failed", ex);
		} finally {
			try {
			...
			for (String fileName: filePosMap.keySet()) {
				...
				try {
				    ...
				    for (Long pointer: filePosMap.get(fileName)) {
					    ...
					    JSONObject obj = JSONObject.fromObject(new String(buffer.array(), StandardCharsets.UTF_8));
					    retList.add(obj);
				    }
				} catch (Exception ex) {
					logger.error("Read cache from file failed", ex);
	...
	public void writeCommitsToCache(String srcStartCommitId, JSONArray content) {
		...
		try {
			...
			Collections.sort(content, new Comparator<JSONObject>() {
				@Override
			...
			if (fileNames == null || fileNames.length == 0) { // 缓存文件不存在
				...
				// 文件已超过大小，创建新文件
				if (oldFile.length() >= CACHE_MAX_SIZE) {
					try {
					...
					String oldContent = FileUtils.readFileToString(oldFile, StandardCharsets.UTF_8);
					
					if (StringUtils.isNotBlank(oldContent)) {
						JSONArray arr = JSONArray.fromObject(oldContent);
						content.addAll(arr);
					}
					
					FileUtils.writeStringToFile(oldFile, content.toString(), StandardCharsets.UTF_8);
	...
	public void updateSVNCommitsToCache(String srcStartCommitId, JSONArray content) {
		try {
			...
			if (fileNames == null || fileNames.length == 0) { // 缓存文件不存在
				...
				if (StringUtils.isNotBlank(oldContent)) {
					...
					while(oldIt.hasNext()) {
						...
						if (StringUtils.equals(o.getString("commitId"), n.getString("commitId"))) {
							saveContent.add(n);
							oldIt.remove();
							newIt.remove();
							
							if (newIt.hasNext()) {
								n = (JSONObject)newIt.next();
							} else {
								break;
							}
						}
					...
					if (arr.size() > 0) {
						saveContent.addAll(arr);
					}
					
					if (content.size() > 0) {
						saveContent.addAll(content);
					}
			...
			if (saveContent.size() > 0) {
				...
				if (file.exists()) {
					if (needChangeName) {
						file.renameTo(new File(commitPathStr + srcCommitIdNum));
					}
				} else {
					if (!path.exists()) {
						if (!path.mkdirs()) {
				...
				Collections.sort(saveContent, new Comparator<JSONObject>() {
					@Override
					public int compare(JSONObject o1, JSONObject o2) {
						return ((Long)o2.getLong("commitId")).compareTo((Long)o1.getLong("commitId"));
					}
				...
				FileUtils.writeStringToFile(file, saveContent.toString(), StandardCharsets.UTF_8, false);
			}
		} catch (Exception ex) {
	...
	public JSONArray getSVNCommitsFromCache(String srcStartCommitId, String srcEndCommitId, int size) {
		try {
			...
			while(idx >= 0 && srcEndCommitIdNum >= commitIdList.get(idx) && commits.size() < size) {
				String content = FileUtils.readFileToString(new File(commitPathStr + commitIdList.get(idx)), StandardCharsets.UTF_8);
				if (StringUtils.isNotBlank(content)) {
					JSONArray arr = JSONArray.fromObject(content);
					if (commits.size() + arr.size() <= size) {// e.g. 100 20 115
						commits.addAll(arr);
					} else {
						for (int i = 0; i < size -commits.size(); i++) {
	...
	public void writeSVNDiffToCache(String leftCommitId, String rightCommitId, JSONArray content) {
		try {
			...
			if (lastestCacheFile.length() >= CACHE_MAX_SIZE) { // 超过最大缓存限制，创建新文件
